...
{
	int lenName = 0;
	char *listCode = "A-CHRDw87lNS0E9B2TibgpnMVys5XzvtOGJcYLU+4mjW6fxqZeF3Qa1rPhdKIouk";
	int i, sum, _char;
	...
	res[0] = listCode[(lenName ^ 59) & 63];
	for (i = 0, sum = 0; i < lenName; i++)
		sum += argv[1][i];
	res[1] = listCode[(sum ^ 79) & 63];
	for (i = 0, sum = 1; i < lenName; i++)
		sum *= argv[1][i];
	res[2] = listCode[(sum ^ 85) & 63];
	_char = argv[1][0];
	for (i = 0; i < lenName; i++)
	{
		if (_char < argv[1][i])
	...
	srand(_char ^ 14);
	res[3] = listCode[rand() & 63];
	for (i = 0, sum = 0; i < lenName; i++)
		sum = sum + argv[1][i] * argv[1][i];
	res[4] = listCode[(sum ^ 239) & 63];
	for (i = 0, sum = 0; i < argv[1][0]; i++)
		sum = rand();
	res[5] = listCode[(sum ^ 229) & 63];
	res[6] = '\0';
	printf("%s\n", res);
	return (0);
}
